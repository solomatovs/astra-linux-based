#!/bin/bash
# сборка динамического модуля ngx_http_auth_ldap_module (аутентификация в LDAP/AD).
...
echo ">>> рантайм-библиотеки openldap"
for p in libldap liblber; do
    cp -a /usr/local/lib/$p.so* "$DEST/usr/local/lib/" 2>/dev/null || true
done

# сниппет load_module
echo 'load_module modules/ngx_http_auth_ldap_module.so;' \
    > "$DEST/etc/nginx/modules.d/20-auth-ldap.conf"

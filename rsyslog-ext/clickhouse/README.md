...
В каждом файле из `conf.d/` в шапке показано, какая строка лога приходит на
вход и что из неё получается в таблице — значения сняты с работающей связки,
не придуманы.
...
* `template` объявляется до действия, которое на него ссылается, иначе
  `Could not find template - action disabled`; порядок `ruleset` и `input`,
  наоборот, свободен.
...
`conf.d/all-in-one.conf.example` — все три сценария сразу (обычный syslog,
файлы как есть, файлы с разбором JSON) одним файлом в `/etc/rsyslog.d/`.
Дробить незачем: `/etc/rsyslog.d/*.conf` всё равно склеивается в одну
конфигурацию. При склейке всплывают четыре правила, все проверены:

1. один и тот же модуль нельзя загрузить дважды — второй `module(load=...)`
   даёт `already in this config, cannot be added`;
2. `template` объявляется до действия, которое его берёт, иначе
   `Could not find template - action disabled` и конфигурация не проходит;
3. `ruleset`, наоборот, может стоять и после `input`, который его вызывает —
   ссылки разрешаются в конце разбора;
4. `queue.filename` уникален на каждое действие с дисковой очередью, иначе
   `already used. This is not possible`.

Проверено: `rsyslogd -N1` проходит начисто, и в одном прогоне из этого файла
одновременно наполнились все три таблицы — 1 строка syslog в `SystemEvents`,
3 строки из трёх локаций в `FileEvents`, 2 в `AppLogs` (разобранный json с
`fields.code=500` и откат в `raw`).

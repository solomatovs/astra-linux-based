...
| `in_probe` | пробы доступности снаружи | tcp (open/closed/filtered), icmp, tls с разбором сертификата, http |
...
```sh
make sources    # исходники: fluent-bit, openssl, libyaml, postgres, ccronexpr
make dist       # -dist образ
make build      # итоговый образ
make test       # проверки собранного образа
make all        # всё перечисленное по порядку
```

`make sources` складывает всё внешнее в `ci-artifacts/fluent-bit/src`, и дальше
`docker build` берёт только оттуда — интернета для тарболов не нужно. Сборочные
пакеты (`flex`, `bison`, `m4`, `pkg-config`, `libsystemd-dev`, `libkrb5-dev`)
ставятся штатным apt: на машине сборки подложен `sources.list` с прокси nexus.
...
### Расписание вместо интервала

`Interval_Sec` задаёт частоту опроса, а `Schedule` — **моменты**, когда прогон
вообще разрешён. Пишется как в cron:
...
Пять полей — привычный cron (минута, час, день, месяц, день недели). Шесть —
первым идёт поле секунд, тогда доступны вещи вроде `*/20 * * * * *` (каждые
двадцать секунд). Пять полей плагин дополняет нулевой секундой сам.

Часовой пояс берётся из переменной `TZ` контейнера, по умолчанию UTC:
...
Как это устроено: коллектор продолжает тикать по `Interval_Sec`, а расписание
на каждом тике решает, работать сейчас или пропустить. Отсюда правило —
**`Interval_Sec` должен быть не реже минуты**, иначе момент можно проспать;
плагин это проверяет и предупреждает в логе:

```
[ warn] interval_sec 300 больше минуты: моменты расписания можно проспать,
        поставьте 60 или меньше
[ info] расписание «0 2 * * *», ближайший прогон 2026-08-27 02:00:00 MSK
```

Пропущенные моменты **не навёрстываются**: следующий считается от текущего
времени. Если fluent-bit стоял сутки, после старта будет один прогон, а не
двадцать четыре подряд.

Ошибку в выражении плагин ловит на старте, а не молча игнорирует:

```
[error] schedule: нужно 5 полей (как в cron) или 6 (с секундами), а их 4: 0 2 * *
[error] schedule не разобрано (Specified range exceeds maximum): 99 2 * * *
```

Разбор выражений — библиотека [ccronexpr](https://github.com/staticlibs/ccronexpr)
(Apache 2.0). Она не ставится в систему: два файла качаются в `ci-artifacts` по
тегу из `DEPS_<версия>` и компилируются рядом с каждым плагином.

Параметр есть у всех трёх входов — `postgres`, `clickhouse` и `pg2ch`. Чаще
всего он нужен последнему: тяжёлый перегон незачем гонять днём. Но и отчётные
запросы вроде `bloat.sql` или `visibility.sql` разумно ставить на ночь.

Когда расписание не нужно:

- **окно, а не момент** — например «работать только с 02:00 до 04:00, но часто»:
  это дешевле выразить прямо в SQL, `AND now()::time BETWEEN '02:00' AND '04:00'`;
- **процесс не должен висеть между прогонами** — тогда честнее внешний cron или
  systemd timer, который запускает контейнер, а `out_exit` завершает его после
  первого сброса.
...
Одновременные запросы не задваивают данные: пока идёт одна выдача, остальные
получают пусто. Проверено залпом из восьми параллельных `curl`: все записи
получил первый, остальные семь — пусто, дублей нет.

Важно, чего замок **не** делает: он не держится на время отправки по сети.
Иначе застрявший клиент подвесил бы и приём данных — кольцо перестало бы
принимать записи из конвейера. Поэтому под замком только снимок и снятие
отданного, а очищается ровно то, что ушло в ответ: пришедшее за время отправки
остаётся в кольце.
...
## Известная особенность 4.0.14

Образ `dmp/fluent-bit:4.0.14` примерно один запуск из десяти падает с SIGSEGV в
`flb_engine_shutdown()` — **уже после** того, как работа сделана, на выходе из
процесса:

```
configuration test is successful
[engine] caught signal (SIGSEGV)
#0  flb_engine_shutdown()
#1  flb_destroy()
```

Это дефект самой версии, а не наших плагинов: воспроизводится на голом
`fluent-bit --dry-run -i dummy -o null` без единого внешнего `.so`. На остальных
трёх версиях сборки не воспроизводится.

Практических последствий для сбора нет — падение происходит при завершении, —
но код возврата становится ненадёжным. Поэтому `test/fluentbitcheck.sh` судит по
тексту ответа, а не по коду возврата; в CI с этой версией стоит делать так же.

## in_probe: пробы доступности

Чёрный ящик: проверяет сервис снаружи, как это делают nmap, ping, `openssl
s_client` и curl. Один плагин, тип задаётся параметром.

```ini
[INPUT]
    Name    probe
    Tag     metric.probe.ports
    Type    tcp
    Targets pg-1:5432, ch-1:8123, dc-1:389, dc-1:636
    Mode    metrics
    Interval_Sec 30
```

Имена метрик намеренно как у **blackbox_exporter** — готовые дашборды и алерты
работают без переписывания:

```
probe_success{instance="samba-ad:636",type="tls"} 1
probe_duration_seconds{instance="samba-ad:636",type="tls"} 0.009213997
probe_ssl_earliest_cert_expiry{instance="samba-ad:636",type="tls"} 1842534988
probe_ssl_cert_expiry_days{instance="samba-ad:636",type="tls"} 633.88
probe_ssl_verify_result{instance="samba-ad:636",type="tls"} 20
probe_port_state{instance="samba-ad:9999",type="tcp"} 0
probe_icmp_duration_seconds{instance="samba-ad",type="icmp"} 0.000829
probe_http_status_code{instance="http://api/health",type="http"} 200
```

`probe_port_state` — наше дополнение поверх blackbox: **1** open, **0** closed,
**2** filtered. Различаются так же, как у nmap: сервер ответил — open, отказал
(RST) — closed, промолчал до таймаута — filtered. Проверено на стенде: открытый
порт 0.000 с, закрытый 0.000 с, «чёрная дыра» — ровно `Timeout_Ms`.

### Четыре типа

| `Type` | что делает | цели |
|---|---|---|
| `tcp` | состояние порта | `host:port` |
| `icmp` | эхо-запрос | `host` |
| `tls` | рукопожатие + разбор сертификата | `host:port` |
| `http` | запрос и проверка ответа | `http://host/path`, `https://…` |

**icmp** берёт непривилегированный ping-сокет (`SOCK_DGRAM`/`IPPROTO_ICMP`) и
откатывается на raw, только если `net.ipv4.ping_group_range` его не пускает —
для raw нужен `CAP_NET_RAW`, который docker и так даёт по умолчанию.

**tls** отдаёт версию протокола, срок и субъект сертификата, результат проверки
цепочки. `Verify off` не отключает проверку, а лишь перестаёт считать её провал
неудачей пробы: `probe_ssl_verify_result` приезжает в любом случае (18 —
самоподписанный, 20 — нет доверия к издателю, 0 — всё в порядке).

Срок истечения приезжает в трёх видах — под алерт, под дашборд и под глаза:

```json
{"instance":"samba-ad:636","type":"tls","success":1,"duration_seconds":0.008952094,
 "failure":"","tls_version":"TLSv1.3",
 "cert_subject":"/O=Samba Administration/OU=Samba - temporary autogenerated HOST certificate/CN=DC1.loshara.com",
 "cert_not_after":"2028-05-21T15:16:28Z","cert_expiry_days":633.83,"verify_result":20}
```

| где | что | для чего |
|---|---|---|
| `probe_ssl_earliest_cert_expiry` | unix-время, `1842534988` | имя как у blackbox: готовые алерты и панели Grafana (она сама рисует это датой) |
| `probe_ssl_cert_expiry_days` | `633.83` | порог в понятных единицах: `< 14` — пора продлевать |
| `cert_not_after` в записи | `2028-05-21T15:16:28Z` | читаемая дата в журнале, UTC, ISO-8601 |

Все три — одно и то же значение. Оно не приблизительное: `ASN1_TIME_diff` даёт
разницу с текущим моментом в днях **и секундах**, точность до секунды.
Проверено сверкой с `openssl x509 -enddate` на двух живых сертификатах —
двухдневном самоподписанном у PostgreSQL и трёхлетнем у контроллера домена:
`notAfter=May 21 15:16:28 2028 GMT` против нашего `2028-05-21T15:16:28Z`.

### Преамбулы: postgres и Active Directory

Тут главная тонкость. К PostgreSQL и к LDAP на 389 порту **нельзя просто
подключиться по TLS** — сначала переговоры:

```ini
    # PostgreSQL: 8 байт SSLRequest, сервер отвечает S или N
    Type     tls
    Targets  pg-1:5432
    Preamble postgres

    # Active Directory на 389: операция расширения LDAP StartTLS
    Type     tls
    Targets  dc-1:389
    Preamble ldap-starttls

    # LDAPS на 636 — обычный TLS, преамбула не нужна
    Type     tls
    Targets  dc-1:636
```

Проверено на живых сервисах: PostgreSQL с `ssl=on` и настоящий контроллер
домена (samba-ad) — обе преамбулы приводят к TLSv1.3 и разбору сертификата.
Преамбула против неподходящего порта даёт честный отказ по таймауту, а не
зависание: «postgres отказал в TLS (ответ не S)».

### Параметры

| параметр | по умолчанию | что делает |
|---|---|---|
| `Type` | `tcp` | `tcp`, `icmp`, `tls`, `http` |
| `Targets` | — | цели через запятую |
| `Timeout_Ms` | `5000` | сколько ждать; по нему же порт признаётся отфильтрованным |
| `Mode` | `metrics` | `metrics`, `logs`, `both` |
| `Metrics_Tag` | — | тег метрик в режиме `both` |
| `Preamble` | `none` | `postgres`, `ldap-starttls` |
| `Verify` | `on` | считать ли непроверенный сертификат неудачей |
| `Ca_File`, `Servername` | — | корневой сертификат и имя для SNI |
| `Method` | `GET` | `GET`, `POST`, `PUT`, `HEAD` |
| `Body`, `Headers` | — | тело и заголовки (`Ключ: значение; Ключ2: значение2`) |
| `Expect_Status` | — | `200`, список `200,204` или класс `2xx`; пусто — любой 2xx/3xx |
| `Expect_Body` | — | подстрока, которая должна быть в ответе |
| `Schedule` | — | cron-выражение вместо интервала |
| `Interval_Sec` | `30` | период проб |

Вход работает в своём потоке (`FLB_INPUT_THREADED`): пробы блокирующие, и
недоступная цель на таймауте не должна задерживать остальной конвейер.

Чего пока нет: IPv6 (адреса разрешаются как IPv4), UDP-проб и проверки
отпечатка сертификата.

## Пара stream: соединение отдельно, данные отдельно

Задача: собрать логи с периферийных серверов в центр, когда сеть пускает
трафик только в одну сторону. Направление данных и направление соединения —
разные вещи, и это главное свойство пары.

**Имя плагина говорит только про данные**, как везде в fluent-bit: `out_stream`
отдаёт, `in_stream` принимает — всегда. Кто кому звонит, задаётся не именем, а
конфигурацией:

| в конфигурации задан | что делает плагин |
|---|---|
| `Listen` (+ `Port`) | слушает и ждёт, когда придут |
| `Host` (у выхода) / `Targets` (у входа) | подключается сам |

Отсюда две схемы; выбирается та, которую пускает межсетевой экран:

```ini
# ── доступ есть только ИЗ центра ─────────────────────────────────
# периферия                        центр
[OUTPUT]                           [INPUT]
    Name    stream                     Name    stream
    Listen  0.0.0.0                    Targets edge-1:10000, edge-2:10000
    Port    10000

# ── доступ есть только В центр ───────────────────────────────────
# периферия                        центр
[OUTPUT]                           [INPUT]
    Name    stream                     Name    stream
    Host    central                    Listen  0.0.0.0
    Port    10000                      Port    10000
    Node    edge-1
```

Данные в обоих случаях текут из `out_stream` в `in_stream`. Меняется только
то, кто установил соединение — и, соответственно, чей порт открыт.

У входа `Targets` и `Listen` можно задать **вместе**: часть узлов центр
обзванивает сам, часть приходит к нему, и всё это один блок `[INPUT]`.
Проверено — в конвейер приезжают и те, и другие, каждый со своей меткой:

```
[input:stream:stream.0] подключаюсь к отправителям: целей 1, по токену
[input:stream:stream.0] жду отправителей на 0.0.0.0:10001, по токену, мест 63
[input:stream:stream.0] edge-a:10000: подключён
[input:stream:stream.0] edge-b подключился с 172.21.0.4:39100
```

У выхода так нельзя: соединение у него одно, поэтому `Listen` вместе с `Host`
плагин отвергает, а не выбирает молча за вас.

```ini
# ── на каждом периферийном сервере ──────────────────────────────
[SERVICE]
    storage.path          /var/lib/fluent-bit/buf
    storage.max_chunks_up 64          # потолок буфера: 64 × 2 МБ ≈ 128 МБ
...
[INPUT]
    ...
    storage.pause_on_chunks_overlimit on    # на потолке ВСТАТЬ, а не терять
...
[OUTPUT]
    ...
    Host     clickhouse
    Database logs
    Query    INSERT INTO logs.edge SELECT parseDateTime64BestEffort(timestamp), source, app, msg FROM input('timestamp String, source String, app String, msg String')
```

Штатными плагинами так не выйдет. Все серверные входы fluent-bit только
принимают — `in_http` на GET отвечает `invalid HTTP method` (проверено), а
единственный вход-клиент, `prometheus_scrape`, умеет только метрики. Обойтись
`in_exec` с curl тоже нельзя: рантайм — голый glibc, в образе нет ни curl, ни
wget, ни python.

### Почему это лучше опроса кольца

`out_ring` копит сам, в памяти, и теряет старое при переполнении. `out_stream`
не копит **ничего**: пока к порту никто не подключён, он отвечает конвейеру
`FLB_RETRY`, и накоплением занимается штатный движок чанков fluent-bit. При
`storage.type filesystem` это диск, а перезапуск периферии ничего не теряет.
Задержка тоже не зависит от периода опроса: пачка уезжает, как только готова.

Проверено счётом пронумерованных строк: центр убирали на 8 секунд —
**200 из 200**, без потерь и дублей; периферию перезапускали при отсутствующем
центре — **300 из 300**.

### Что происходит, когда центр долго не приходит

Это главный вопрос всей схемы, и ответ **не** «плагин копит, пока хватает
места». `in_tail` про потребление не знает ничего: он читает, пока его не
остановит движок, а движок смотрит только на переполнение буфера. Что именно
он сделает — зависит от настройки, и разница принципиальная (все цифры
измерены на файле из 40 000 строк при отсутствующем центре):

| настройка периферии | поведение | итог |
|---|---|---|
| `storage.total_limit_size 2M` у выхода | **выбрасывает самые старые чанки**, вход продолжает читать | доставлено 18 184, потеряно 21 816 |
| `storage.type filesystem` без потолка | пишет на диск, вход не тормозит | потерь нет, но диск растёт неограниченно |
| `Mem_Buf_Limit 1M` у входа | **вход встаёт** | 40 000 из 40 000 |
| filesystem + `storage.pause_on_chunks_overlimit on` + `storage.max_chunks_up 8` | **вход встаёт**, диск замер на ~14 МБ | 200 000 из 200 000 (проверено на 40 МБ) |

`storage.total_limit_size` — это **порог выбрасывания**, а не размер страховки:
```
[error] [input chunk] fail to drop enough chunks in order to place new data
        coming from input plugin tail.0
```

Правильное поведение даёт пауза входа:
```
[warn] [input] tail.0 paused (storage buf overlimit 8/8)
[info] [input] pausing tail.0
      ... центр появился ...
[info] [input] tail.0 resume (storage buf overlimit 7/8)
```

Пока вход на паузе, он **не читает файл и не двигает отметку в своей базе** —
буфером становится сам файл на диске. Отсюда два следствия:

- потолок (`storage.max_chunks_up` × 2 МБ) надо брать с запасом на ожидаемое
  время недоступности центра, но он ограничивает только буфер fluent-bit —
  дальше данные ждут в самом файле;
- **ротация опаснее переполнения**: если файл повернётся и старый будет удалён,
  пока вход стоит на паузе, эти данные не прочитает уже никто. Запас по числу
  файлов ротации должен перекрывать самый долгий простой центра.

Оговорка про остановку: если в момент **остановки периферии** `in_tail` успел
прочитать строки, но задача на отправку ещё не создана, эти строки теряются —
отметка в его базе уже сдвинута. Это окно самого `in_tail`, к транспорту
отношения не имеющее: в том же опыте периферия отправила 190 записей и центр
принял ровно 190, а до конвейера не дошли 10.

### Протокол

Строчный, поэтому отлаживается руками через `openssl s_client`:

```
центр →   FLBSTREAM 1 <токен>        токен «-», если не задан
периф →   OK   либо   ERR <причина> и разрыв
периф →   BATCH <номер> <байт>\n<JSON-строки, по записи на строку>
центр →   ACK <номер>                после того, как записи ушли в конвейер
периф →   PING                       при простое
центр →   PONG
```

Про гарантии честно: `ACK` означает «центральный принял в свой конвейер», а не
«записано в СУБД» — сквозного подтверждения от выхода ко входу в fluent-bit нет
ни у кого. Пачка без `ACK` остаётся чанком и уедет повтором, поэтому семантика
доставки — не реже одного раза.

### Метка источника

В записи появляется поле `source` (имя задаётся `Source_Key`). Что в нём:

- центр звонил сам → строка из `Targets`, как написана: `edge-1:10000`;
- периферия звонила → её `Node`, а если он не задан — адрес, с которого
  пришла. Адрес меняется от подключения к подключению, поэтому `Node` в этой
  схеме стоит задать всегда.

### Один потребитель за раз

Пока периферия отдаёт одному центру, второй подключившийся получает разрыв
сразу после accept, а в журнале периферии появляется «подключение отклонено:
уже отдаю центру …». Иначе пачки поехали бы двум получателям вперемешку.

Грабля, найденная запуском: сперва отказ вообще не выдавался. Поток принимал
соединения только когда был свободен, и пока шла отдача, новые соединения
копились непринятыми в очереди прослушивания — подключившийся висел до своего
таймаута, не получая ни отказа, ни разрыва.

### TLS и токен

TLS терминирует сама периферия, nginx не нужен:

```ini
    # тот, кто СЛУШАЕТ — свой сертификат
    Secure     On
    Cert_File  /etc/ssl/edge.crt
    Key_File   /etc/ssl/edge.key
    # тот, кто ЗВОНИТ — чем проверять чужой
    Secure     On
    Ca_File    /etc/ssl/ca.crt
```

Обратите внимание: набор параметров зависит не от того, `in` это или `out`, а
от того, кто слушает. Пароль спрашивает тоже слушающий — его порт открыт
наружу, ему и защищаться.

Переключатель называется `Secure`, а не `Tls`, потому что свойства на `tls*`
ядро перехватывает раньше `config_map` — свой параметр с таким именем просто не
сработает. Внутри — напрямую OpenSSL, как в `in_probe`: `flb_tls_session_create`
требует внутренний `flb_connection`, собирать который вручную для своего сокета
хрупко. Серверная половина символов из бинарника экспортирована (проверено
`nm -D`: `TLS_server_method`, `SSL_accept`, `SSL_CTX_use_certificate_chain_file`).

Токен проверяется на рукопожатии и сравнивается целиком, без раннего выхода из
цикла — чтобы длину совпадающего префикса нельзя было измерить по времени
ответа.
...
| параметр | по умолчанию | что делает |
|---|---|---|
| `Listen` + `Port` | — | адрес прослушивания. Задан — ждём получателя |
| `Host` + `Port` | — | адрес получателя. Задан — звоним сами (свойства ядра) |
| `Node` | — | имя, которым представляемся, когда звоним: станет меткой источника |
| `Token` | — | пароль: у пришедших спрашиваем, при своём подключении предъявляем |
| `Retry_Pause_Sec` | `10` | пауза между попытками, когда звоним сами |
| `Require_Ack` | `on` | ждать подтверждения пачки |
| `Ack_Timeout_Ms` | `10000` | сколько ждать, прежде чем считать соединение мёртвым |
| `Heartbeat_Sec` | `30` | период `PING` при простое; 0 — не слать |
| `Handshake_Ms` | `5000` | сколько ждать строку рукопожатия |
| `Secure` | `off` | включить TLS |
| `Cert_File`, `Key_File` | — | свой сертификат и ключ — когда слушаем |
| `Ca_File`, `Verify`, `Servername` | —, `on`, — | проверка чужого сертификата — когда звоним |
| `Time_Key`, `Time_Format` | `timestamp`, `iso8601` | как класть время события в запись |

`in_stream` (центр):

| параметр | по умолчанию | что делает |
|---|---|---|
| `Targets` | — | отправители через запятую `host:port` — к ним ходим сами |
| `Listen` + `Port` | — | адрес прослушивания — сюда приходят сами. Можно вместе с `Targets` |
| `Node` | — | имя, которым представляемся, когда звоним сами |
| `Token` | — | пароль: у пришедших спрашиваем, при своём подключении предъявляем |
| `Source_Key` | `source` | поле с именем периферии; пустое — не добавлять |
| `Time_Key` | `timestamp` | поле со временем события; **поле забирается во время события и в теле не дублируется** |
| `Poll_Ms` | `200` | период опроса сокетов |
| `Retry_Pause_Sec` | `10` | пауза перед повторным подключением |
| `Connect_Timeout_Ms` | `5000` | таймаут подключения и рукопожатия |
| `Batch_Max_Bytes` | `64M` | защита от испорченного заголовка пачки |
| `Secure` | `off` | включить TLS |
| `Ca_File`, `Verify`, `Servername` | —, `on`, — | проверка чужого сертификата — когда звоним |
| `Cert_File`, `Key_File` | — | свой сертификат и ключ — когда слушаем |

Про `Time_Key` отдельно: разобранное поле в тело записи **не копируется**.
Время у события своё, отдельно от полей, и выход на той стороне добавит его
сам — оставь мы копию, в JSON оказалось бы два ключа `timestamp`, на что
ClickHouse отвечает `Duplicate field found while parsing JSONEachRow`
(наступали).

Чего пока нет: IPv6, нескольких одновременных потребителей у одного выхода и
сжатия потока. Если у входа включён `Secure` и заданы сразу `Listen` и
`Targets`, TLS работает только на приёме — контекст OpenSSL один; плагин про
это предупреждает в журнале.
...
`seed.sh` попутно включает TLS у postgres: генерирует самоподписанный
сертификат и переводит `ssl` в `on` перечитыванием конфигурации — рестарт не
нужен, у параметра контекст `sighup`. Без живого рукопожатия проверять пробу
`Type tls` было бы не на чем: `check.sh` сверяет дату истечения сразу в трёх
местах — метрика, запись и `openssl x509 -enddate` самого сертификата — и они
обязаны совпасть до секунды.
...
В стенде есть и вторая, «периферийная» служба `edge`: она слушает 10000 и ждёт,
когда за логами придёт центральный fluent-bit стенда. `check.sh` проверяет
не только что записи доехали с меткой источника, но и что при остановленном
центре периферия копит на диск, а после его возвращения доливает.
